...
/**
 * Filter function to check if a date is after the given value
 * For ISO format dates like "2023-03-15T12:30:45.000Z"
 */
const after: LogicFunction = (value: any, compareWith?: any) => {
    ...
    try {
        ...
        // Value date should be after compare date
        return d1.getTime() >= d2.getTime();
    } catch (e) {
        console.error('Date after filter error:', e);
        return false;
    }
};

// Set extra property to 'datepicker' so the filter knows to use a datepicker
after.extra = 'datepicker';
